...
estimate.bic.surv.cpen = function(formula, data,r = 1.0/457.0,logn=log(457.0),relat =c("to25","expi","logi","to35","troot","sigmoid"))
{
  #print(names(data))
  mlik = NULL
  toret = NULL
...
for(j in 100:1)
{
  ...
  res1 = pinferunemjmcmc(s.seed = j,predict = F,  test.data = NULL, n.cores = 32, report.level =  0.1, num.mod.best = 1000 ,simplify = T,link.function = g,
                         runemjmcmc.params = list(formula = formula1,outgraphs=F,data = data.example,estimator = estimate.bic.surv.cpen,  
                                                  ...
                                                  save.beta = T,interact = T,relations=c("to25","expi","logi","to35","troot","sigmoid"),
                                                  relations.prob = rep(0.1,6),
                                                  interact.param=list(deep.method=1,allow_offsprings=3,mutation_rate = 250,last.mutation = 15000, max.tree.size = 5, Nvars.max =15,p.allow.replace=0.9,p.allow.tree=0.01,p.nor=0.9,p.and = 0.9),n.models =  20000,unique = F,
  ...
  write.csv(x =res1$feat.stat,row.names = F,file = paste0("posteriorsurvBGNLM_",j,".csv"))
  print(paste0("end simulation ",j))
  
  #print the run's metrics and clean the results
  write.csv(file =paste0("resultsurvrunBGNLM_",j,".csv"),x= results[,j,])
  #rm(results)
...
for(j in 1:100)
{
  results[1:2,j,1:5] = as.matrix(read.csv(paste0("resultsurvrunBGNLM_",j,".csv"))[,-1])
}
