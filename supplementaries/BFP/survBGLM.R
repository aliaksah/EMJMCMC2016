...
estimate.bic.surv.cpen = function(formula, data,r = 1.0/457.0,logn=log(457.0),relat1=c("p0","p2","p3","p05","pm05","pm1","pm2"),relat2=c("p0p0","p0p05","p0p1","p0p2","p0p3","p0p05","p0pm05","p0pm1","p0pm2"))
{
  #print(names(data))
  mlik = NULL
  toret = NULL
...
for(j in 1:100)
{
  ...
  res1 = pinferunemjmcmc(s.seed = j,predict = F,  test.data = NULL, n.cores = 2, report.level =  0.1, num.mod.best = 1000 ,simplify = T,link.function = g,
                         runemjmcmc.params = list(formula = formula1,outgraphs=F,data = data.example,estimator = estimate.bic.surv.cpen,  
                                                  ...
                                                  save.beta = T,interact = F,gen.prob = c(5,3,1,0,0),relations=c("p0","p2","p3","p05","pm05","pm1","pm2","p0p0","p0p05","p0p1","p0p2","p0p3","p0p05","p0pm05","p0pm1","p0pm2"),
  ...
  preds.second =  predict.surv.bma(res.object = list(threads.stats = list(res1$threads.stats[[2]])),data.train = data.example, data.test = data.test)
  
  preds.first =  predict.surv.bma(res.object = list(threads.stats = list(res1$threads.stats[[1]])),data.train = data.example, data.test = data.test)
  ...
  results[1,j + 50,1]=   preds.second$ibs[1]
  results[1,j + 50,2]=   preds.second$ibs[2]
  results[1,j +50,3]=   preds.second$cindex$coxph  
  
  
  write.csv(x =res1$feat.stat,row.names = F,file = paste0("posteriorsurvBLint_",j,".csv"))
  print(paste0("end simulation ",j))
  
  #print the run's metrics and clean the results
  write.csv(file =paste0("resultsurvrunBL_",j,".csv"),x= results[,j,])
  #rm(results)
...
for(i in 1:2){
  for(j in 1:5)
  {
    summary.results[i,(j-1)*3+1]=min(results[i,,j],na.rm = T)
    summary.results[i,(j-1)*3+2]=median(results[i,,j],na.rm = T)
    summary.results[i,(j-1)*3+3]=max(results[i,,j],na.rm = T)
  }
}
...
for(j in 1:100)
{
  tmpf = read.csv(paste0("posteriorsurvFPint_",j,".csv"))
...
write.csv(x =tmp,row.names = T,file = "survivalfeatFPint.csv")
#print(paste0("end simulation ",j))
