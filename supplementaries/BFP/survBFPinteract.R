...
for(j in 1:100)
{
  ...
  res1 = pinferunemjmcmc(s.seed = j,predict = F,  test.data = NULL, n.cores = 32, report.level =  0.1, num.mod.best = 1000 ,simplify = T,link.function = g,
                         runemjmcmc.params = list(formula = formula1,outgraphs=F,data = data.example,estimator = estimate.bic.surv.cpen,  
                                                  ...
                                                  save.beta = T,interact = T,gen.prob = c(5,3,1,0,0),relations=c("p0","p2","p3","p05","pm05","pm1","pm2","p0p0","p0p05","p0p1","p0p2","p0p3","p0p05","p0pm05","p0pm1","p0pm2"),
  ...
  write.csv(x =res1$feat.stat,row.names = F,file = paste0("posteriorsurvFPint_",j,".csv"))
  print(paste0("end simulation ",j))
  
  #print the run's metrics and clean the results
  write.csv(file =paste0("resultsurvrunint_",j,".csv"),x= results[,j,])
  #rm(results)
...
for(j in 1:100)
{
  print(file.info(paste0("resultsurvrunint_",j,".csv"))$ctime)
  results[1:2,j,1:5] = as.matrix(read.csv(paste0("resultsurvrunint_",j,".csv"))[,-1])
}
