...
estimate.gamma.cpen = function(formula, data,r = 1.0/3177.0,logn=log(3177.0),relat1=c("p0","p2","p3","p05","pm05","pm1","pm2"),relat2=c("p0p0","p0p05","p0p1","p0p2","p0p3","p0p05","p0pm05","p0pm1","p0pm2"))
{
  fparam=NULL
...
for(j in 1:100)
{
  ...
  res1 = pinferunemjmcmc(s.seed = j, n.cores = M, report.level =  0.2 , num.mod.best = NM,simplify = T, predict = T,
                         ...
                         runemjmcmc.params =list(formula = formula1,data = data.example,
                                                 ...
                                                 relations.prob = rep(0.1,16),interact.param=list(allow_offsprings=3,mutation_rate = 100,
                                                                                                  last.mutation=10000, max.tree.size = 4, Nvars.max = 15,p.allow.replace=0.5,p.allow.tree=0.3,p.nor=0.3,p.and = 0.9),
  ...
  write.csv(x =res1$feat.stat,row.names = F,file = paste0("posteriorshellFP_",j,"_interact.csv"))
  print(paste0("end simulation ",j))
  
  #print the run's metrics and clean the results
  write.csv(file =paste0("resultsrun_",j,"_interact.csv"),x= results[,j,])
  #rm(results)
...
for(j in 1:100)
{
  ...
  tmp = read.csv(paste0("resultsrun_",j,"_interact.csv"))
  
  results[1,j,1]=  tmp[1,2]
  results[1,j,2]=  tmp[1,3]^2
...
for(j in 1:100)
{
  tmpf = read.csv(paste0("posteriorshell_",j,"_interact.csv"))
...
write.csv(x =tmp,row.names = T,file = "abalonefeatFP_interact.csv")
