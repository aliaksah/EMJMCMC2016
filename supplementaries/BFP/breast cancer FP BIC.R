...
for(j in 1:100)
{
  ...
  res1 = pinferunemjmcmc(s.seed = j, n.cores = M, report.level =  0.2 , num.mod.best = NM,simplify = T, predict = T,
                         ...
                         runemjmcmc.params =list(formula = formula1,data = data.example,
                                                 ...
                                                 recalc_margin = 249, save.beta = T,interact = T,gen.prob = c(5,0,1,0,0),relations=c("p0","p2","p3","p05","pm05","pm1","pm2","p0p0","p0p05","p0p1","p0p2","p0p3","p0p05","p0pm05","p0pm1","p0pm2"),
                                                 relations.prob = rep(0.1,16),interact.param=list(allow_offsprings=3,mutation_rate = 100,
                                                                                                  last.mutation=10000, max.tree.size = 2, Nvars.max = 45,p.allow.replace=0.5,p.allow.tree=0.3,p.nor=0.3,p.and = 0.9),
  ...
  write.csv(x =res1$feat.stat,row.names = F,file = paste0("posteriorbreastcancerFPBIC_",j,".csv"))
  print(paste0("end simulation ",j))
  
  #print the run's metrics and clean the results
  write.csv(file =paste0("resultsrunbreastcancerBIC_",j,".csv"),x= results[,j,])
  #rm(results)
...
for(j in 1:100)
{
  ...
  tmp = read.csv(paste0("resultsrunbreastcancerBIC_",j,".csv"))
...
{
  print(j)
  tmpf = read.csv(paste0("posteriorbreastcancerFPBIC_",j,".csv"))
...
tmp = simplifyposteriors(th=0.0001,thf=0.1, X = data.example,post =as.data.frame(cbind(keys(featgmj),as.numeric(values(featgmj)))),y = "X")

write.csv(x =cbind(tmp[1:15,c(2,1)],tmp[16:30,c(2,1)]),row.names = F,file = "breastcancerfeatFP1.csv",sep = "&")
#print(paste0("end simulation ",j))
